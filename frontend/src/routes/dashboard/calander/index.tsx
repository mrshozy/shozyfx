...
import Table, {DataTableColumnConfig, DataTableColumnHeader} from "../../../components/table";
import {z} from "zod";
...
export default function Calender() {
    ...
    setFixed(true)
    onChange("Calender")
    const [data, setData] = useState<any>(events_mock)
    const [loading, setLoading] = useState(data.length == 0)
    // const loadData = (start: Date, end: Date) => {
    //   let url = `/calender?start_date=${formattedDate(start)}&end_date=${(formattedDate(end)).toString()}`;
    //   axiosInstance.get(url)
    //       .then(d => d.data).then(json => setData(json.data)).then(()=>setLoading(false))
    //       .catch(e => {
    //         toast({
    ...
    // useEffect(()=>{
    //   if(loading){
    //     loadData(new Date(), addDays(new Date(), nextFriday()+1))
    //   }
    // }, [])
    //
    // const onDateChange =  (rage: DateRange|undefined) => {
    //   if (rage != undefined) {
    //     if (rage.from && rage.to) {
    //       setLoading(true)
    //       loadData(rage.from!, rage.to!)
    //     }
    //   }
    // }
    return (
